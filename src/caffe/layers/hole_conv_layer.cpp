...
#include "caffe/layers/hole_conv_layer.hpp"

namespace caffe {

template <typename Dtype>
void HoleConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (!conv_param.has_hole_h()) {
    if (conv_param.has_hole()) {
      hole_h_ = hole_w_ = conv_param.hole();
    } else {
      CHECK(false) << "No hole paramter set. Use Convolution instead.";
    }
  } else {
...
template <typename Dtype>
void HoleConvolutionLayer<Dtype>::compute_output_shape() {
...
template <typename Dtype>
void HoleConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  CHECK_EQ(4, bottom[0]->num_axes()) << "Input must have 4 axes, "
      << "corresponding to (num, channels, height, width)";
  num_ = bottom[0]->num();
...
template <typename Dtype>
void HoleConvolutionLayer<Dtype>::forward_cpu_gemm(const Dtype* input,
...
template <typename Dtype>
void HoleConvolutionLayer<Dtype>::forward_cpu_bias(Dtype* output,
    const Dtype* bias) {
...
template <typename Dtype>
void HoleConvolutionLayer<Dtype>::backward_cpu_gemm(const Dtype* output,
...
template <typename Dtype>
void HoleConvolutionLayer<Dtype>::weight_cpu_gemm(const Dtype* input,
...
template <typename Dtype>
void HoleConvolutionLayer<Dtype>::backward_cpu_bias(Dtype* bias,
    const Dtype* input) {
...
template <typename Dtype>
void HoleConvolutionLayer<Dtype>::forward_gpu_gemm(const Dtype* input,
    const Dtype* weights, Dtype* output, bool skip_im2col) {
  const Dtype* col_buff = input;
  if (!is_1x1_) {
    if (!skip_im2col) {
...
template <typename Dtype>
void HoleConvolutionLayer<Dtype>::forward_gpu_bias(Dtype* output,
    const Dtype* bias) {
...
template <typename Dtype>
void HoleConvolutionLayer<Dtype>::backward_gpu_gemm(const Dtype* output,
    const Dtype* weights, Dtype* input) {
...
template <typename Dtype>
void HoleConvolutionLayer<Dtype>::weight_gpu_gemm(const Dtype* input,
    const Dtype* output, Dtype* weights) {
  const Dtype* col_buff = input;
  if (!is_1x1_) {
    conv_im2col_gpu(input, col_buffer_.mutable_gpu_data());
...
template <typename Dtype>
void HoleConvolutionLayer<Dtype>::backward_gpu_bias(Dtype* bias,
    const Dtype* input) {
...
template <typename Dtype>
void HoleConvolutionLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void HoleConvolutionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(HoleConvolutionLayer);
#endif

INSTANTIATE_CLASS(HoleConvolutionLayer);
REGISTER_LAYER_CLASS(HoleConvolution);
