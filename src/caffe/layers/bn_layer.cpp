...
void BNLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  // extract param
  use_history_in_test_ = this->layer_param_.bn_param().use_history_in_test();
  ...
  if (this->blobs_.size() > 0) {
    ...
    // this->blobs_[2]->SetLearnable(false);

    // history variance
    this->blobs_[3].reset(new Blob<Dtype>(shape));
    caffe_set(channels_, frozen_ ? Dtype(1) : Dtype(0), this->blobs_[3]->mutable_cpu_data());
    // this->blobs_[3]->SetLearnable(false);
  ...
  // shutdown scale and bias update in global stat mode
  if (this->frozen_){
    // slope
    this->layer_param_.mutable_param(0)->set_lr_mult(float(0));
    this->layer_param_.mutable_param(0)->set_decay_mult(float(0));

    // bias
    this->layer_param_.mutable_param(1)->set_lr_mult(float(0));
    this->layer_param_.mutable_param(1)->set_decay_mult(float(0));
  }
}
...
void BNLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (!this->frozen_ && this->phase_ == TRAIN) {
    caffe_cpu_axpby(batch_statistic_.count(), decay_, batch_statistic_.cpu_data(), Dtype(1) - decay_,
        this->blobs_[3]->mutable_cpu_data());
  }
  if (this->phase_ == TEST && this->use_history_in_test_) {
    // use history variance
