...
void BaseConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  /*
  CHECK((!conv_param.has_hole() && conv_param.has_hole_h() && conv_param.has_hole_w())
  ...
  */
  ...
  if (!conv_param.has_hole_h()) {
    ...
    if(!conv_param.has_hole_w()) {
      CHECK(false) << "NO hole parameter set. Use Convolution instead"; 
    } else {
  ...
  group_compute_in_parallel_ = this->layer_param_.convolution_param().group_compute_in_parallel();
  ...
  if (this->blobs_.size() > 0) {
    ...
    // }
#ifdef USE_MPI
    caffe_mpi_bcast(this->blobs_[0].get(), DATA_MEM, this->blobs_[0]->count()); 
#endif
    ...
    if (bias_term_) {
      ...
      // }
#ifdef USE_MPI
      caffe_mpi_bcast(this->blobs_[1].get(), DATA_MEM, this->blobs_[1]->count()); 
#endif
    }
  }
...
void BaseConvolutionLayer<Dtype>::backward_gpu_bias(Dtype* bias,
    const Dtype* input) {
  caffe_gpu_gemv<Dtype>(CblasNoTrans, num_output_, height_out_ * width_out_, (Dtype)1.,
