...
template <>
void col2im_gpu<float>(const float* data_col,
    const int num, const int channels, const int height, const int width,
    const int kernel_h, const int kernel_w, const int pad_h, const int pad_w,
    const int stride_h, const int stride_w, const int hole_h, const int hole_w,
    float* data_im) {
  ...
  caffe_gpu_set(num * channels * height * width, float(0), data_im);
  col2im_gpu_kernel<float><<<CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS>>>(
...
template <>
void col2im_gpu<double>(const double* data_col,
    const int num, const int channels, const int height, const int width,
    const int kernel_h, const int kernel_w, const int pad_h, const int pad_w,
    const int stride_h, const int stride_w, const int hole_h, const int hole_w,
    double* data_im) {
  ...
  int num_kernels = num * channels * height_col * width_col;
  // We will use atomicAdd in the kernel to accumulate the values.
  caffe_gpu_set(num * channels * height * width, double(0), data_im);
  col2im_gpu_kernel<double><<<CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS>>>(
      num_kernels, data_col,
      num, channels, height, width,
      kernel_h, kernel_w, pad_h, pad_w,
      stride_h, stride_w, hole_h, hole_w,
