...
def disp_kitti_error(td, d):
    """
    Calculate average end point error
    :param td: ground-truth disp map
    :param d:  predict disp map
    :return: End point error of the estimated disparity
    """
    tau = [3, 0.05]
    '''
    stu = tu[bord+1:end-bord,bord+1:end-bord]
...
def disp_to_color(disp, max_d = -1):
    """
    Convert disp into kitti color code image
    :param flow: disparity map
    :return: colorized disparity map
    """
    d = disp.flatten()
    # print disp.shape
    # print d.shape
    ...
    color_bins  = color_map[:-1, 3].astype(np.float)
    color_cbins = np.cumsum(color_bins).astype(np.float)
    color_bins  = color_bins / color_cbins[-1]
    color_cbins = color_cbins[:-1] / color_cbins[-1]

    # ind = min(sum(repmat(I(:)', [6 1]) > repmat(cbins(:), [1 numel(I)])),6) + 1;
    # color_ind   = np.min(np.sum( (np.tile(np.transpose(d) ,[6, 1]) > np.transpose((np.tile(color_cbins, [d.size, 1])))), 0 ), 6) + 1
    color_ind = np.sum( (np.tile(np.transpose(d), [6, 1]) > np.transpose((np.tile(color_cbins, [d.size, 1])))), 0 )
    color_ind[color_ind > 6] = 6
    color_bins  = 1 / color_bins
    color_cbins = np.append([0.0], color_cbins)

    d = (d - color_cbins[color_ind]) * color_bins[color_ind]
    d = color_map[color_ind, 0:3] * np.transpose(np.tile(1-d, [3, 1])) + color_map[color_ind+1, 0:3] * np.transpose(np.tile(d, [3, 1]))
    d[d > 1.0] = 1.0
    d[d < 0.0] = 0.0
    d = np.reshape(d, (disp.shape[0], disp.shape[1], 3))
    color_disp = (d * 255.0).astype(np.uint8)
    color_disp = cv2.cvtColor(color_disp, cv2.COLOR_RGB2BGR)

    # kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3,3))
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5,5))
    color_disp = cv2.dilate(color_disp, kernel, iterations=1)

    return color_disp

def disp_error_to_color(gt_disp, pd_disp):
    tau = [3, 0.05]
    color_cols  = np.array([[0/3.0,         0.1875,     49,     54,     149],
                            ...
                            [24/3.0,        48,         215,    48,     39],
                            [48/3.0,        np.inf,     165,    0,      38]], dtype = np.float)
    error_disp = np.fabs(gt_disp - pd_disp)
    divide_disp = gt_disp + 1e-5
    error_disp = np.minimum( (error_disp / tau[0]), (error_disp / np.fabs(divide_disp) / tau[1]))
    error_disp[gt_disp == 0] = 0.0
    error_map = np.zeros( (gt_disp.shape[0], gt_disp.shape[1], 3) , dtype = np.uint8)
    for i in range(color_cols.shape[0]):
        error_index = np.logical_and(error_disp > color_cols[i, 0], error_disp <= color_cols[i, 1])
        error_map[:, :, 0][error_index] = color_cols[i, 2]
        error_map[:, :, 1][error_index] = color_cols[i, 3]
        error_map[:, :, 2][error_index] = color_cols[i, 4]
    color_error = cv2.cvtColor(error_map, cv2.COLOR_RGB2BGR)
    # kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3,3))
    # color_error = cv2.dilate(color_error, kernel, iterations=1)
    return color_error
...
def evaluate_kitti_flow(gt_flow, pred_flow):
    # print gt_flow.shape
...
def evaluate_kitti_disp(gt_disp, pred_disp):
    tau = [3, 0.05]
    index_valid = (gt_disp != 0)
    n_total     = np.sum(index_valid)
    # print stu.size
    # print n_total

    epe = np.abs(gt_disp - pred_disp)
    mag = np.abs(gt_disp) + 1e-5
...
def evaluate_occ_kitti_disp(gt_noc_disp, gt_all_disp, pred_disp):
    tau = [3, 0.05]
    # occ_valid = (gt_noc_disp == 0)
    # label_valid = (gc_all_disp != 0)
    index_valid = np.logical_and((gt_noc_disp == 0), (gt_all_disp != 0))
    # index_valid = (gt_disp != 0)
    n_total     = np.sum(index_valid)
    if n_total == 0:
        mean_epe = 0.0
        ...
        epe = np.abs(gt_all_disp - pred_disp)
        mag = np.abs(gt_all_disp) + 1e-5
...
def read_disp_png(file_name):
    # disp_img = cv2.imread(file_name, cv2.CV_LOAD_IMAGE_UNCHANGED)
    disp_img = cv2.imread(file_name, cv2.IMREAD_UNCHANGED)
...
def resize_disp(disp, des_width, des_height):
    src_width   = disp.shape[1]
    ratio_width     = float(des_width) / float(src_width)
    disp = cv2.resize(disp, (des_width, des_height), interpolation=cv2.INTER_NEAREST)
